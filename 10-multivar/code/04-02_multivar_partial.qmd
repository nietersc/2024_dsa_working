...
Class note on the PC score - multiply the actual site year data by the eigenvalue coefficients to predict where that site year would fall
...
mod_km <- kmeans(weather_norm,
                 centers = 6,
                 nstart = 10 #how many times to run kmeans analysis runs, returns best results
                 )
...
#plot show is often called the elbow method - identify where "elbow" visually appears. So in this plot, somewhere between 3 and 4
```
...
#looking at average silhouette width that is the highest - 
