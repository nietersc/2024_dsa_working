...
**Note on R-squared vs adjusted R squared - R-squared will always increase as more variables are added, adjusted R squared penalizes additional variables added if they explain very little of the variance!**
...
mod4_ar1 <- nlme::lme(
  ...
  data = rm_rcbd_df) #Phi1 = 0, tells us correlation = 0. 
...
#very similar to default model 1, mod1 is just assuming correlation = 0. 
...
#mod 4 consumes degree of freedom, but doesn't improve fit, which increases AIC and BIC
...
mod5_arma11
# has Phi (autoregressive part) and Theta for correlation
...
ACF(mod5_arma11, resType = "n") %>%
  plot(alpha = 0.01)
#Does not help us fixing or reducing auto correlations according to this plot
...
#if low and upper includes 0, it is not significantly different than 0
#this is true for Phi and Theta 
...
#has worse fit compared to default as it consumes 2 degrees of freedom for Phi and Theta
#this gives lower AIC and BIC scores

#these are all serial correlation structures, next we get into spatial correlation data
...
Semivariogram and correlation will be inverse - highly correlated means the distances will be closest to eachother.Range is the point on the x axis where the curve flattens out.
...
#plot won't show line because it was unable to fit an exponential model to the points as there are few time points, not many distances and they have a relatively flat response shape. 
...
#correlation structure tells in table tells us the plateau will occur somewhere between 0 and 30
#this encompasses nearly the full range of daps so it's not very useful.
...
#again, no fit of the points in the plot
...
#correlation structure is from 6 - 16. Quite wide, but better than previous spatial model
...
#still not best model according to AIC and BIC
...
#intervals don't include 0, but upper limit is 120 which isn't in  our actual dap range
#^ means this method is not a good fit
...
```
range interval does not include 0, so it is significant. But upper limit is 90 so no bueno
