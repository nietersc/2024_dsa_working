...
reg_dfw |>
  ggplot(aes(x = factor(sr_ksha), y = yield_mgha,
             color = factor(sr_ksha)))+
  geom_boxplot()
...
#looks like a quadtratic response
#loess response of geom_smooth allows for the fit to be "wiggly"
...
# Model fitting
# select a package that allows for mixed-effect models (lme4)
mod1_int <- lme4::lmer(yield_mgha ~ (1 | rep),
               data = reg_dfw)
...
#residuals are increasing across the x axis, but error band is still encompassing 0
...
#plots looking at normality of blocks (4)
...
#include regression fit of model we created
#can use geom_smooth, but it becomes more difficult with
...
#note - name of variables in df below MUST match whats in the dataframe we are comparing bootstrapping to
...
nd <- nd |>
  mutate(
    mod1_yield_mgha = predict(mod1_int,
                                   nd,
                  #re.form tells fxn not to include rep
                                   re.form = NA))
...
mod1_int_boots <- lme4::bootMer(mod1_int, 
                          predict.fun, 
                          nsim = 200) %>% #bootstrapping 200 times, use at least 1000 for our own research
  stats::confint() %>%
  as.data.frame() %>%
...
# Model fitting
mod2_lin <- lme4::lmer(yield_mgha ~ (1 | rep) + sr_ksha,
                       data = reg_dfw)
...
# Model fitting
mod4_linp <- nlme::nlme(yield_mgha ~ nlraa::SSlinp(
  sr_ksha,a,b,xs),
  data = reg_dfw,
...
mod4_linp |> intervals(which = "fixed")
...
boot <- reg_dfw |>
  infer::specify(response = yield_mgha) |>
  infer::generate(reps = 10000,
                  type = "bootstrap")
